...
export default function CustomerEdit({ customer }) {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        put(`/cadastros/clientes/${customer.id}`);
    }
    ...
    return (
        <>
            ...
            <div className="space-y-6 max-w-3xl">
                <div>
                    <Link href={`/cadastros/clientes/${customer.id}`} className="text-xs text-[var(--color-primary)] hover:underline">
                        ← Voltar para {customer.name}
                    </Link>
                    <h1 className="text-xl font-bold text-[var(--color-text)] mt-1">Editar Cliente</h1>
                </div>
